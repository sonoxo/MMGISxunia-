...
async function cachedFetch(key,url,ttl=60000,headers={}){
  ...
  const r=await fetch(url,{headers:{'user-agent':'XUNIAverse-Planetary-Forge/2.1',accept:'application/json',...headers}});if(!r.ok)throw new Error(`upstream ${r.status}`);const data=await r.json();cache.set(key,{at:Date.now(),data});return data;
}
...
async function radioGeo(){const raw=await cachedFetch('radio','https://de1.api.radio-browser.info/json/stations/search?hidebroken=true&order=clickcount&reverse=true&limit=500',300000,{'user-agent':'XUNIAverse/2.1'});const features=[];for(const s of raw||[]){const lat=finite(s.geo_lat),lon=finite(s.geo_long);if(lat==null||lon==null)continue;features.push({type:'Feature',geometry:{type:'Point',coordinates:[lon,lat]},properties:{id:s.stationuuid,title:s.name,country:s.country,state:s.state,language:s.language,tags:s.tags,homepage:s.homepage,source:'Radio Browser',_feed:'radio'}})}return{type:'FeatureCollection',features}}
...
async function dataCenterGeo(){const url='https://www.peeringdb.com/api/fac?limit=1000&fields=id,name,city,country,latitude,longitude,net_count,ix_count,carrier_count,website,updated';const raw=await cachedFetch('datacenters',url,900000);const features=[];for(const d of raw.data||[]){const lat=finite(d.latitude),lon=finite(d.longitude);if(lat==null||lon==null||Math.abs(lat)>90||Math.abs(lon)>180)continue;features.push({type:'Feature',geometry:{type:'Point',coordinates:[lon,lat]},properties:{id:d.id,title:d.name,city:d.city,country:d.country,networks:d.net_count||0,exchanges:d.ix_count||0,carriers:d.carrier_count||0,website:d.website||'',updated:d.updated||'',source:'PeeringDB',_feed:'datacenters'}})}return{type:'FeatureCollection',features,meta:{provider:'PeeringDB',kind:'public interconnection facilities',count:features.length}}}
async function feed(req,res,url){
  ...
  try{
    ...
    if(id==='datacenters')return json(res,200,await dataCenterGeo());
